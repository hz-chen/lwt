...
/* 
 * Prof. Parmer's output on an Intel Core i5-2520M CPU @ 2.50GHz:
 *
 * [PERF] 120 <- fork/join
 * [PERF] 13 <- yield
...
test_perf(void)
{
	...
	rdtscll(start);
	for (i = 0 ; i < ITER2 ; i++) {
		pthread_create(&tid, NULL, fn_null_pthd, NULL);
		pthread_join(tid, NULL);
	}
	rdtscll(end);
	printf("[PERF] %lld <- pthread_fork/join\n", (end-start)/ITER2);
	*/
...
test_perf_channels(int chsz)
{
	...
	assert(_LWT_STAT_RUN == curr_tcb->lwt_status);
	from = lwt_chan(chsz);
	assert(from);
	//lwt_chan_grant(from);
