...
fn_bounce(void *d, lwt_chan_t c) 
{
	...
	lwt_yield(_LWT_NULL);
	rdtscll(start);
	for (i = 0 ; i < ITER ; i++) lwt_yield(_LWT_NULL);
	rdtscll(end);
	lwt_yield(_LWT_NULL);
	lwt_yield(_LWT_NULL);
...
test_perf(void)
{
	...
	chld1 = lwt_create(fn_bounce, (void*)1, 0, 0);
	chld2 = lwt_create(fn_bounce, NULL, 0, 0);
	lwt_join(chld1);
	lwt_join(chld2);
	IS_RESET();
}
...
fn_nested_joins(void *d, lwt_chan_t c)
{
	...
	chld = lwt_create(fn_nested_joins, (void*)1, 0, 0);
	lwt_join(chld);
	return NULL;
}
...
test_crt_join_sched(void)
{
	...
	chld1 = lwt_create(fn_sequence, (void*)1, 0, 0);
	chld2 = lwt_create(fn_sequence, (void*)2, 0, 0);
	lwt_join(chld2);
	lwt_join(chld1);	
	IS_RESET();
	...
	chld1 = lwt_create(fn_nested_joins, NULL, 0, 0);
	lwt_join(chld1);
	IS_RESET();

	/* functional tests: join only from parents */
	chld1 = lwt_create(fn_identity, (void*)0x37337, 0, 0);
	chld2 = lwt_create(fn_join, (void*)chld1, 0, 0);
	lwt_yield(_LWT_NULL);
	lwt_yield(_LWT_NULL);
	lwt_join(chld2);
	lwt_join(chld1);
...
test_perf_channels(int chsz)
{
	...
	assert(_LWT_STAT_RUN == lwt_lst_root[current_tid].lwt_status);
	from = lwt_chan(chsz);
	assert(from);
	//lwt_chan_grant(from);
...
void *
fn_snder(void *arg, lwt_chan_t pppc)
{
...
test_multisend(int chsz)
{
	...
	t1 = lwt_create(fn_snder, &args[0], 0, 0);
	t2 = lwt_create(fn_snder, &args[1], 0, 0);
	for (i = 0 ; i < ITER*2 ; i++) {
		//if (i % 5 == 0) lwt_yield(_LWT_NULL);
		ret[i] = (int)lwt_rcv(c);
...
test_perf_async_steam(int chsz)
{
	...
	assert(from);
	//lwt_chan_grant(from);
	t = lwt_create(fn_async_steam, from, 0, 0);
...
void *
fn_grpwait(void *d, lwt_chan_t ppc)
{
	lwt_chan_t c = d;
	int i;
	...
	for (i = 0 ; i < ITER ; i++) {
		if ((i % 7) == 0) {
			...
			for (j = 0 ; j < (i % 8) ; j++) lwt_yield(_LWT_NULL);
		}
		lwt_snd(c, (void*)current_tid);
	}
	return NULL;
}
...
test_grpwait(int chsz, int grpsz)
{
	...
	for (i = 0 ; i < grpsz ; i++) {
		...
		assert(cs[i]);
		//lwt_chan_grant(cs[i]);
		ts[i] = lwt_create(fn_grpwait, cs[i], 0, 0);
	...
	for (i = 0 ; i < ((ITER * grpsz)-(grpsz*chsz)) ; i++) {
		...
		c = lwt_cgrp_wait(g, LWT_CHAN_RCV);
		assert(c);
		r = (int)lwt_rcv(c);
